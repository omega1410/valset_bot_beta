...
def register_section_handlers(app: Client):
     ...
     async def back_to_section(client: Client, callback_query: CallbackQuery):
         ...
         async def set_photo_name(client, message):
             """Устанавливаем имя для следующего фото"""
             if len(message.command) < 2:
                 await message.reply(
                     "❌ Укажите имя файла:\n"
                     "`/upload_photo my_photo.png`\n\n"
                     "После этого отправьте фото.",
                     parse_mode="Markdown"
                 )
                 return
         ...
         async def save_photo_to_assets(client, message):
             ...
             file_path = await client.download_media(message.photo, file_name=f"assets/{filename}")
